...
def float2datetime(time, date=datetime.datetime.utcnow()):
    ...
    return datetime.datetime(
        date.year, date.month, date.day, hh, mm, ss, ms, tzinfo=pytz.timezone("UTC")
    )
...
def getData(lang_module):
    ...
    sun = lib.Sun.Sun()
    sunrise, sunset = sun.sunRiseSet(
        now.year, now.month, now.day, config.location[0], config.location[1]
    )
    ...
    if (
        now > sunrise
        and (
            config.hoursBeforeSunRise == -1
            or (now - sunrise).seconds / 3600.0 <= config.hoursBeforeSunRise
        )
    ) or config.giveSunRiseAfterSunRise == 1:
        data["data"] = " ".join(
            (
                data["data"],
                lang.sunrise,
                lang.readHour(sunrise.astimezone(pytz.timezone(config.timeZone))),
            )
        )
    ...
    if (
        sunset > now
        and (
            config.hoursBeforeSunSet == -1
            or (sunset - now).seconds / 3600.0 <= config.hoursBeforeSunSet
        )
    ) or config.giveSunSetAfterSunSet == 1:
        data["data"] = " ".join(
            (
                data["data"],
                lang.sunset,
                lang.readHour(sunset.astimezone(pytz.timezone(config.timeZone))),
            )
        )

    if config.giveDayLength == 1:
        data["data"] = " ".join(
            (data["data"], lang.dayLength, lang.readHourLen(sunset - sunrise))
        )
