...
def parse_csv():
    ...
    with open(config.file_path, "rb") as csvfile:
        ...
        for row in reader:
            row[2] = datetime.datetime.strptime(row[2], "%Y-%m-%d %H:%M:%S").strftime(
                "%s"
            )
            try:
                db.execute(
                    ...
                    values (?,?,?,?,?,?, (select max(timestamp) from radathome))""",
                    row,
                )
                added += 1
    ...
    debug.log(
        "RADATHOME",
        "Added %d, not added %d measurements"
        % (
            added,
            not_added,
        ),
...
def getData(l):
    ...
    if rlevel is not None:
        ...
        data["data"] = lang.removeDiacritics(
            " ".join(
                (lang.radiation_level, lvl, lang.readFraction(rlevel, 3), lang.uSiph)
            )
        )
