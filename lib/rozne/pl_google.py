...
def cardinal(no, units=["", "", ""], gender="M"):
    ...
    if no < 0:
        return (
            ("minus " + cosslownie(-no, units, plec=gender))
            .replace("jeden tysiąc", "tysiąc", 1)
            .encode("utf-8")
        )
    else:
        return (
            cosslownie(no, units, plec=gender)
            .replace("jeden tysiąc", "tysiąc", 1)
            .encode("utf-8")
        )
...
def readHourLen(hour):
    ...
    mm = (ss - hh * 3600) / 60
    return removeDiacritics(
        " ".join((cardinal(hh, hrs, gender="F"), cardinal(mm, mns, gender="F")))
    )
...
hscr_region = {
    "K": "w_karkonoszach obowiazuje",
    "J": "w_jesionikach_i_masywie_snieznika obowiazuje",
}
avalancheLevel = [""] + [
    i + " stopien_zagrozenia_lawinowego"
    for i in ["pierwszy", "drugi", "trzeci", "czwarty", "piaty_najwyzszy"]
]
...
avalancheLevel = [""] + [
    i + " stopien_zagrozenia_lawinowego"
    for i in ["pierwszy", "drugi", "trzeci", "czwarty", "piaty najwyzszy"]
]
