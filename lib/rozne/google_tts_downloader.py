...
for word in dictionary.download_list:
    ...
    if not os.path.exists("%s.ogg" % filename):
        ...
        url = (
            '"http://translate.google.com/translate_tts?tl=%s&q=%s&total=1&idx=0&client=t"'
            % (dictionary.LANGUAGE, urllib.parse.quote_plus(phrase + " ."))
        )
        os.system(
            "wget -q -U 'Mozilla/5.0 (X11; Linux i686) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/44.0.2403.107 Safari/537.36' -O %s.mp3 %s"
            % (filename, url)
        )
        ...
        length = float(
            subprocess.Popen(
                ["soxi", "-D", "%s.wav" % filename], stdout=subprocess.PIPE
            ).communicate()[0]
        )

        os.system(
            "sox %s.wav %s.ogg trim %s %s"
