...
class taf:
    ...
    def __init__(self, ICAO=None, at=(0, 0, 0), taf=None, address=None):
        ...
        _wind = re.compile(
            "(VRB|\d{3})(\d{2,3})(?:(G)(\d{2,3}))?(KT|MPS|KMH)(?:(\d{3})(V)(\d{3}))?"
        )
        _vis = re.compile("(?:^|\s)\d\d\d\d(?:$|\s)|P?\dSM")
        _weather = re.compile(
            "^(?:\+|-|VC)?(?:MI|BC|DR|BL|SH|TS|FZ)?(?:(?:DZ|RA|SN|SG|IC|PE|GR|GS)|(?:BR|FG|FU|VA|DU|SA|HZ)|(?:PO|SQ|FC|SS|DS))$"
        )
        _skyCnd = re.compile("(SKC|FEW|SCT|BKN|OVC|NSC)(\d\d\d(VV\d\d\d)?)?(CB|TCU)?")
        _temp = re.compile(
            """\s(M)?\d\d/(M)?\d\d\s| # METAR
                                 T(M)?\d\d/?\d\dG # TAF"""
        )
            ...
            if _valid.search(line) is not None:
                ...
                for word in line.split(" "):
                    tl["period"] = (
                        tl["period"]
                        + " "
                        + " ".join((self.flatten(_period.findall(word)) or ""))
                    ).strip()
                    tl["wind"] = (
                        tl["wind"]
                        + " "
                        + " ".join((self.flatten(_wind.findall(word)) or ""))
                    ).strip()
                    tl["vis"] = (
                        tl["vis"]
                        + " "
                        + " ".join((self.flatten(_vis.findall(word)) or ""))
                    ).strip()
                    tl["weather"] = (
                        tl["weather"]
                        + " "
                        + " ".join((self.flatten(_weather.findall(word)) or ""))
                    ).strip()
                    tl["temp"] = (
                        tl["temp"]
                        + " "
                        + " ".join((self.flatten(_temp.findall(word)) or ""))
                    ).strip()
                    tl["press"] = (
                        tl["press"]
                        + " "
                        + " ".join((self.flatten(_press.findall(word)) or ""))
                    ).strip()
                    tl["skyCnd"] = (
                        tl["skyCnd"]
                        + " "
                        + " ".join((self.flatten(_skyCnd.findall(word)) or ""))
                    ).strip()
                    tl["cavok"] = (
                        tl["cavok"]
                        + " "
                        + " ".join((self.flatten(_cavok.findall(word)) or ""))
                    ).strip()
                    tl["nsw"] = (
                        tl["nsw"]
                        + " "
                        + " ".join((self.flatten(_nsw.findall(word)) or ""))
                    ).strip()
        ...
        for i in range(0, len(self.tafData)):
            ...
            if (
                self.tafData[i]["cavok"] == "CAVOK"
                and "".join(
                    (
                        self.tafData[i]["vis"],
                        self.tafData[i]["weather"],
                        self.tafData[i]["skyCnd"],
                    )
                )
                == ""
            ):
                        ...
                        if elem in w:
                            if self._WeatherConditions[k][elem] == "%":
                                print(
                                    " ".join(
                                        (
                                            "---> lib/taf.py: couldn't interpret",
                                            elem,
                                            "in",
                                            self.weather["weather"].strip().split(),
                                        )
                                    )
                                )
                            else:
                                if self._WeatherConditions[k][elem][1] != "":
                                    ...
                                    self._WeatherConditions[k][elem][0] != ""
                                    and self._WeatherConditions[k][elem][0] not in wx[0]
                                ):
                                    wx[0] = " ".join(
                                        (wx[0], self._WeatherConditions[k][elem][0])
                                    )
